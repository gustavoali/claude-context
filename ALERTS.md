...
|-------|------|---------|--------|
| 2026-03-28 | recordatorio | Quimera (`pj qm`): infra dev OK (Docker, migrations, Groq API). Pendiente: test e2e via API, seed plans, frontend + backend conectados, consulta legal | Sesion dedicada: uvicorn + frontend, probar flujo register->profile->generate |
| 2026-03-28 | **ALTA PRIORIDAD** recordatorio | **MCP Shared Gateway (`pj msg`):** implementado. 6 servers via mcp-proxy, ahorro medido 1.6 GB (63%). Scripts Start/Stop/Status listos. Pendiente: activar en ~/.claude.json y validar E2E con sesion real | Activar: (1) `pwsh Start-Gateway.ps1`, (2) aplicar config HTTP (docs/claude-json-config.md), (3) reiniciar sesiones. Registrar en PA DB |
